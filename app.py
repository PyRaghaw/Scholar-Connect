...
def upload_document():
    """Handle document upload with PDF support - UPDATED"""
    ...
    try:
        ...
        student_data = extract_data(text)
        matched_scholarships = match_scholarships(student_data)
        stats = calculate_statistics(matched_scholarships)
...
@app.route('/manual', methods=['POST', 'OPTIONS'])
def manual_entry():
    """Handle manual data entry - SUPPORTS stream & state"""
    ...
    try:
        data = request.json
        logger.info(f"Manual entry received: {data}")
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "error": "Invalid percentage or income format"
            }), 400
        ...
        student_data = {
            ...
            "stream": data.get("stream", "").strip(),  # NEW
            "state": data.get("state", "").strip()      # NEW
        }
...
def get_all_scholarships():
    ...
    try:
        ...
        if category:
            filtered = [s for s in filtered if category in s["category"]]
